...
## [2026-05-22] 추가 | 신제품·인허가 채널 핀고정 v2 게시본 박제 + 자동 봇 누락 사유 명시

**트리거**: 사용자가 5/22 오전 9:12·9:16 슬랙에 게시한 `#qa-신제품-출시검증` · `#qa-인허가-법규` 핀고정 메시지 공유.

**핵심 발견 — 인허가 채널 자동 모니터링 봇 안내 누락**:
- 초안에 있던 "🤖 매일 아침 8~9시 본부장 PC 자동 수집 → 본 채널 게시" 섹션이 실제 게시본에서 **삭제**
- 사유 추정: 본부장 [[안전관리정보_자동수집_제안|PowerShell PoC]] 정식 가동 전이라 보수적 접근 (핀고정에 미리 약속 X)
- 추가 시점: PoC 첫 가동 + 1주 안정도 확인 후 핀고정 편집 (Slack 핀은 편집해도 핀 유지)

**갱신 (1건)**:
- `wiki/industry/LAKA/analysis/품질팀_슬랙채널_규칙_2026-05.md` §2 #qa-신제품-출시검증 + §5 #qa-인허가-법규 — **v2 실제 게시본** 박스 추가, v1 핀고정 공지는 "참고용 보존". 인허가 채널에 ⚠️ 자동 봇 안내 누락 사유 + 추가 시점 명시

→ 클레임(5/22 작성) + 신제품·인허가(5/22 오전) 3채널 실제 게시본 모두 위키 박제 완료. 팀내(Private)는 핀고정 시 추가 박제 예정.

---

## [2026-05-22] 시행 | 품질팀 Slack 4채널 개설 + 클레임 긴급 룰 전환

**트리거**: 사용자가 5/22 마감일 슬랙 4채널 개설 완료. 채널 핀고정 가이드 초안 + 1주 운영 후 가안 단계 명시. 영향 문서 모두 갱신 요청.

**주요 변경 4건**:
1. **채널 5개 → 4개** — `#qa-제조업체-audit` 개설 안 함 (외부 업체 audit 관련 문서를 공용 폴더에서 운영 중). 1주 운영 회고에서 재검토
2. **클레임 채널 긴급 룰 전환** — 식약처 위해정보 / 중대 클레임 / 회수 가능성 / 라인 중단 = 슬랙 멘션 ❌ → **전화·대면 우선** (천승범 010-8905-0366 / 차승호 010-3552-3109 / 김건 010-5634-3274). 슬랙 알림은 외근·미팅·집중 모드에서 놓칠 수 있어 분 단위 긴급 대응 부적합
3. **추천 Slack 템플릿 매트릭스 신설** — 신제품(프로젝트 시작 키트/도움 요청 프로세스) · 클레임(피드백 추적기 ⭐) · 인허가(팀 지원+자동 봇) · 팀내(템플릿 없음). 1주 운영 + 과거 업무 이메일 포워딩 받은 후 확정
4. **(가안) 표기** — 1주 운영 후 확정 표기 제거

**갱신 (5건)**:
- `wiki/industry/LAKA/analysis/품질팀_슬랙채널_규칙_2026-05.md` — **v1 → v2 (5/22 시행본, 가안)** 전면 갱신. v1→v2 변경표 / 추천 템플릿 매트릭스 / `#qa-클레임` 핀고정 메시지 교체 / `#qa-제조업체-audit` 개설 안 함 박스 / 운영 책임 매트릭스 v2 / 에스컬레이션 룰 v2 (긴급 전화·대면 명시)
- `wiki/industry/LAKA/QA팀/품질팀_슬랙운영_초안_2026-05.md` — 상단에 "v2 시행본은 [[품질팀_슬랙채널_규칙_2026-05]] 참조" 안내 박스. status: draft → archived-as-source. 5/22 결정 3건 요약
- `wiki/industry/LAKA/QA팀/Slack_품질팀_시작_가이드.md` — §5 "4채널 中 #qa-팀내만 비밀" 정정 + §6 🚨 긴급 = 전화·대면 신설 (3인 연락처 + 표)
- `wiki/industry/LAKA/QA팀/차승호_1차미팅_아젠다.md` — 아젠다 #2 ✅ 시행 표시 + 4채널 결정 박스
- `wiki/industry/LAKA/QA팀/차승호_2차미팅_아젠다.md` — 아젠다 #6 본부장 1차안 표 4채널 반영 (audit 제외, 공용 폴더 운영) + 5/22 긴급 클레임 룰 추가 박스

**잔여 후속**:
- 1주 운영 (5/22~5/29) 후 회고 — 채널 통폐합·신설 검토, audit 채널 재검토
- 차승호 2차 의제 #6 Q2 "최근 1개월 다른 팀 요청 사례 5~10건" 포워딩 받기 → 추천 템플릿 확정
- 6월 첫 주 회고에서 (가안) 표기 제거 + 정식 시행본 확정

---

## [2026-05-20] 인제스트 | 매직립픽서 2차 CT 사양변경·일정 공유 (차승호 매니저)

**트리거**: 품질팀 [[차승호]] 매니저 → 본부장([[천승범]]) 메시지 (5/20 오전 10:20). 매직립픽서 2차 CT가 차주 콜마 전달 → 5월 말 결과 예정. 런칭 일정 적합성 확인 + 신규 CT 진행 현황 문의. 사용자 "인제스트" 지시.

**핵심 사실**:
- 대상 SKU: [[products/매직-립-픽서|매직 립 픽서]] (KR/JP 양국 운영, ODM = [[한국콜마]])
- 시료 전달: 2026-05-28~29 (차주 목~금) → 결과 회신: 2026-05-말 (예정)
- 2차 CT 변경사양 3건 동시:
  - 재질 **SUS304 → SUS316** (내부식성 상향)
  - 팁 SUS **표면처리 업체 변경**
  - 박킹 **십자절개 추가** (솔대 펌핑 효과 개선)
- 트리거 카테고리: [[CT결과서_폴더_2026]] §4 변경 추적 문서의 표준 케이스(재질·표면처리·박킹 동시 변경)

**시사점**:
- 본부장 답변 시 런칭 KO 일자 → 양산 발주 → CT 결과 데드라인 역산 필요. 부적합 시 3차 CT 여유 기간 확보 여부가 결정 변수.
- 차승호 매니저가 "그 외 신제품 CT" 진행 현황을 본부장에게 물어보는 패턴 = 본부장 측 신제품 CT 트래커 부재 가능성 → [[QA팀_이관업무_2026]] QC #5 신제품 CT 항목과 정합.

**신규 (2건)**:
- `wiki/industry/LAKA/sources/src_매직립픽서_2차CT_260520.md` — 메시지 요약 + 시사점
- `wiki/industry/LAKA/concepts/스테인리스_등급_SUS304_SUS316.md` — SUS304 vs SUS316 비교 (Mo 함량·내식성·CT 재실시 항목·표면처리 별개 축)

**업데이트 (3건)**:
- `wiki/industry/LAKA/products/매직-립-픽서.md` — ODM(한국콜마) 명시, CT 이력 표 신설, sources/tags 보강, SUS 컨셉 링크
- `wiki/industry/LAKA/QA팀/CT결과서_폴더_2026.md` — §4 변경 추적 표에 매직립픽서 3개 변경 행 추가, 표면처리 업체 변경 행 신설, 연결 섹션에 src·entity·SUS 컨셉 페이지 추가
- `wiki/index.md` — src + SUS 컨셉 2건 등록 (총 267페이지)

**raw 파일**: `raw/work/매직립픽서_2차CT_차승호_260520.txt`

---

## [2026-05-20] 인제스트 | 글로벌마케팅팀 R&R (2026-05-19, 안일홍 본부장)

**트리거**: 안일홍 마케팅 부문장이 2026-05-19 Slack에 글로벌마케팅팀 R&R PDF 공유. 사용자 "md로 우선 전환 → 위키 업데이트" 지시.

**핵심 사실**:
- 글로벌마케팅팀 = 리드 3인(이다은·김다빈·박가이 병렬) + 실무 3인(한혜민·하지윤·김태희) = **6명**
- 영역 분담: IMC·SNS·온오프(이다은) / PR·인플루언서·리테일 커뮤니케이션(김다빈) / Meta·TikTok 광고(박가이)
- 실무 분담: 인스타·시딩·물류(한혜민) / DM·캡션·스튜디오(하지윤) / **Threads**·커뮤니티·미디어·현장(김태희)
- **영문 카피 분담선 공식화**: 브랜드 카피 = 김다빈(글로벌마케팅) / **제품 단 카피 = 김정민(글로벌PR팀장)**
- 실질 PM = 이다은 (예산·일정·외부·크로스팀 의사결정 7번 항목)

**[[LAKA_조직도_2026-05]] 대비 변화 (1일 갭)**:
- 조직도(5-18): 글로벌마케팅팀 = 김다빈(팀장) + 박가이·이다은 3명
- R&R(5-19): + **한혜민·하지윤·김태희** 실무 3인 (본 소속은 글로벌PR팀 인턴)
- 해석: 인사 발령이 아닌 **부문 내 매트릭스 배치 공식화**. 글로벌PR팀 인턴 풀(8명)이 마케팅 부문 공용 리소스로 운용되는 구조 확정.

**신규 (2건)**:
- `wiki/industry/LAKA/sources/src_글로벌마케팅팀_RR_260519.md` — 원본 소스 페이지
- `wiki/industry/LAKA/analysis/글로벌마케팅팀_RR_2026-05.md` — R&R 전문 + 채널별 담당 매트릭스 + 조직도 대비 분석

**업데이트 (1건)**:
- `wiki/industry/LAKA/analysis/LAKA_조직도_2026-05.md` — 글로벌마케팅팀 행에 실무 3인 매트릭스 배치 주석 + 관련 위키 링크 추가 + updated 2026-05-20

**raw 파일**: `raw/work/글로벌마케팅팀_RR_260519.md` (PDF → md 전환)

---

## [2026-05-19] 인제스트 | 오션글로벌 신규 entity — 부진재고 TF 3대 채널 매트릭스 완성

**트리거**: 사용자 "오션 글로벌 추가 리서치해봐" 요청. 웹 검색 4종 진행 — 외부 공개 정보 거의 없음 (비공개·소규모 유통사 추정).

**핵심 사실 (PPT + 웹 확인)**:
- 한국 본사 동남아·일본·대만 K-뷰티 유통사
- 시장: **베트남·태국·싱가포르·미얀마·일본·대만**
- 모델: **올영 X 유통 결합 B2B + 라이브 커머스**
- 자매회사: **미흥글로벌** — 중국 동업자가 별도 운영, 해외 수출 인지도
- 입점 브랜드: 엘지·정샘물·브이티·메디큐브·온그리디언츠·성분에디터
- 2026-03 싱가포르 라이브 40회 (10팀 X 4회) 캠페인 운영
- **결정적 인사이트**: "바이어들은 올영 온라인 입점 제품 선호" → **올영 입점 = K-뷰티 글로벌 신뢰 지표** 정량 확인
- 라카 진행: 2/12 통화 → 2/26 사무실 → **3/5 본사 미팅 (오후 2시, [[Adaline_Group]] 오전 11시와 더블헤더)**

**신규 (1건)**:
- `wiki/industry/LAKA/entities/distributors/오션글로벌.md` — 유통 모델 + 미흥글로벌 관계 + 라이브커머스 + TF 매트릭스 완성 + 7건 시사점

**부진재고 TF 3대 채널 매트릭스 완성** ✅:
- 글로벌 컨설팅: [[Adaline_Group]] (한국 본사, 글로벌 유통)
- **동남아·동아시아**: **오션글로벌** (한국 본사, 동남아+일본+대만, 라이브 커머스)
- 국내: [[코아시스]] (현대백화점·아울렛, 부진재고 7종 매칭)

**시장 중복 경고**: 오션글로벌과 정규 B2B 채널 ([[POYA]] 대만 / [[sociolla 소시올라|Sociolla]] 인도네시아 / [[AMID]] 대만) 시장 중복. 라카 측 채널 경합 또는 SKU 분담 정책 필요.

---

## [2026-05-19] 인제스트 | 코아시스 (coasis) 신규 entity — 국내 부진재고 TF 핵심 채널

**트리거**: 사용자가 SCM 2026-04-W3 slide5 (코아시스 진행 건) 이미지 공유, 위키 등록 여부 확인 요청. 검색 결과 6개 파일에서 단편 언급만 있고 **별도 entity 페이지 부재** 확인.

**핵심 사실**:
- 코아시스 (coasis) = **현대백화점·아울렛 입점 색조·라이프뷰티 편집숍**
- 2026 6개 점포 확장 (스페이스원·천호·가든파이브·동대문·목동) + 연중 백화점 팝업 4회
- 주요 입점 브랜드: 브이티(월 매출 1,000만+)·카버코리아·메디필·세포랩
- **라카 제안 예정 7종 SKU 全 [[부진재고_등급제_운영]] B/C등급** (젤링누드글로스·벌키매트립스틱·본딩선크림·본딩커버쿠션·모노아이섀도우·포에버6아이팔레트·클렌징 2종)
- 협상 조건: **올영 상위 매출 제품 제외 거래** (정상채널 vs 세컨더리 분리)

**신규 (1건)**:
- `wiki/industry/LAKA/entities/distributors/코아시스.md` — 점포 확장 일정 + 제안 예정 SKU 매트릭스 (부진재고 등급 매핑) + TF 평행 라인 비교 + 5건 시사점

**부진재고 TF 3대 채널 매트릭스 완성**: [[Adaline_Group]] (글로벌 컨설팅) + 오션글로벌 (동남아) + **코아시스 (국내 부진재고 소진)** = 3대 채널 모두 entity화 완료.
